...
import { tasksSvc, entitiesSvc, maestroProcessesSvc, caseInstancesSvc, casesSvc } from '../lib/sdk';
...
export async function getCaseInstances(processKey: string): Promise<CaseInstance[]> {
  try {
    ...
    return items.filter((raw: any) => raw.processKey === processKey).map((raw: any) => ({
      ...
      startedAt: new Date(raw.startedTimeUtc || raw.creationTime).toLocaleString(),
...
export async function assignTask(taskId: string, folderId: number): Promise<void> {
  ...
  await tasksSvc.assign({
    ...
    userNameOrEmail: import.meta.env.VITE_UIPATH_USER_EMAIL || 'operator@tradeflow.ai'
  });
}
